...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${process.env.BACKEND_URL || 'http://localhost:5000'}/api/requests`, {
      method: 'POST',
      headers: {
...
export async function DELETE(request: Request) {
  ...
  try {
    const response = await fetch(`${process.env.BACKEND_URL || 'http://localhost:5000'}/api/requests?id=${id}`, {
      method: 'DELETE',
    });
